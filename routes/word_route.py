...
async def get_random_word(
    ...
    if body.Length is not None and body.Length > 0:
        word = db.query(Word).where(Word.Length == body.Length).order_by(
            func.rand()).limit(1).first()
    elif body.Max > 0 and body.Min > 0:
        word = db.query(Word).where(
            Word.Length.between(body.Min, body.Max)
