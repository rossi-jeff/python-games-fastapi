...
import random
from utilities.ten_grand import TenGrandScoreOptions, TGCategoryScoreAndDice, RemoveUsedDice, UpdateTenGrand
from models.ten_grand_turn import TenGrandTurn
...
async def ten_grands_in_progress(
    ...
    if user_id is not None:
        ten_grands = db.query(TenGrand).where(
            TenGrand.Status == 1).filter(TenGrand.user_id == user_id).all()
        for tg in ten_grands:
            items.append(tg.as_dict())
    return items
...
async def ten_grand_score(
    ...
    if body.TurnId > 0:
        turn = db.query(TenGrandTurn).where(
            TenGrandTurn.id == body.TurnId).first()
    else:
        db.add(turn)
        db.commit()
        db.refresh(turn)
    ...
    options.sort(
        key=lambda o: TenGrandDiceRequired[o.Category.name], reverse=True)
    crapOut: bool = False
    ...
    db.commit()
    db.refresh(turn)
    UpdateTenGrand(db, ten_grand_id)
    return turn.as_dict()
