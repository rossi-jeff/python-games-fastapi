...
async def code_breakers_in_progress(
    ...
    if user_id is not None:
        code_breakers = db.query(CodeBreaker).where(
            CodeBreaker.Status == 1).filter(CodeBreaker.user_id == user_id).all()
        for cb in code_breakers:
            items.append(cb.as_dict())
    return items
...
async def get_code_breaker_by_id(code_breaker_id: int, db: Session = Depends(get_db)) -> CodeBreakerResponse:
    code_breaker = db.query(CodeBreaker).where(
        CodeBreaker.id == code_breaker_id).first()
...
async def take_code_breaker_guess(
    ...
    for color in body.Colors:
        ...
        db.commit()
    solution = []
    codes = db.query(CodeBreakerCode).where(
        CodeBreakerCode.code_breaker_id == code_breaker_id).all()
    for code in codes:
    ...
    code_breaker = db.query(CodeBreaker).where(
        CodeBreaker.id == code_breaker_id).first()
    count = db.query(CodeBreakerGuess).where(
        CodeBreakerGuess.code_breaker_id == code_breaker_id).count()
    status = CalculateCodeBreakerStatus(black, code_breaker.Columns, count)
    if status != "Playing":
        code_breaker.Status = GameStatusArray.index(status)
        code_breaker.Score = CalculateCodeBreakerScore(
            db, code_breaker_id, code_breaker.Columns, code_breaker.Colors)
        db.add(code_breaker)
        db.commit()
